...
# 支持文件夹（会自动递归传输）和单个文件
FILES_TO_TRANSFER = [
    r"D:\zzr\科研\server_fedavg\client_scalefl.py",      # 示例：更新单个脚本
    r"D:\zzr\科研\server_fedavg\client_scalefl.py",
    r"D:\zzr\科研\server_fedavg\models",                 # 示例：更新整个 models 文件夹
    # r"D:\zzr\科研\server_fedavg\utils\options.py",     # 你可以随时取消注释来添加
]

# 2. 远程目标路径 (所有设备统一放这里)
REMOTE_PATH = "~/server_fedavg/"  # 建议指定到具体项目文件夹，例如 ~/FedProject/

# 3. 设备列表 (IP, 用户名, 密码)
# 建议密码相同时直接复制粘贴，格式：(IP, 用户名, 密码)
DEVICES = [
    # --- Nano (7台) ---
    ("192.168.31.237", "jetsonnano", "your_password"),
    ("192.168.31.238", "jetsonnano", "your_password"),
    ("192.168.31.239", "jetsonnano", "your_password"),
    # ... 把剩下的 Nano 补全 ...
    
    # --- Xavier (2台) ---
    ("192.168.31.240", "xavier_user", "xavier_pass"),
    ("192.168.31.241", "xavier_user", "xavier_pass"),

    # --- Orin (1台) ---
    ("192.168.31.242", "orin_user", "orin_pass"),
]
